...
async def custom_raise(event):
    ...
    # الحصول على النص بعد "رفعع"
    full_text = event.pattern_match.group(1).strip()
    
    if not full_text:
        return
    
    # استخراج أول كلمة للتحقق فقط (مع الاحتفاظ بالنص الكامل للرفع)
    first_word = full_text.split()[0].strip()
    
    # إذا كانت الكلمة الأولى من الأوامر المحجوزة، اتركها للأوامر القديمة
    if first_word in reserved_commands:
        return
    
    # ✅ التعديل هنا: استخدم النص الكامل، وليس فقط الكلمة الأولى
    word = full_text  # كان سابقاً first_word
