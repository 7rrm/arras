...
import re
import random
# إضافة هذه الاستيرادات في بداية الكود الأول
import asyncio
import aiohttp
import random
import json
...
# إضافة هذه الدوال بعد قسم الدوال المساعدة في الكود الأول
...
def create_or_edit_image(session_token, prompt, image_urls=None):
    ...
    cookie_string = f"__Secure-authjs.session-token={session_token}; __Secure-authjs.callback-url=https%3A2F%2Fnanabanana.ai%2Far%2Fai-image"
    headers = {
        'User-Agent': "Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/143.0.0.0 Mobile Safari/537.36",
        'Content-Type': "application/json",
        'sec-ch-ua-platform': "\"Android\"",
...
async def bot_start(event):
    ...
    if chat.id != Config.OWNER_ID:
        ...
        buttons = [
            ...
            [
                Button.inline("تعديل الصور 🎨", data="edit_image"),  # 
            ],
            [
...
# إضافة هذا الأمر بعد @l313l.bot_cmd
@l313l.bot_cmd(pattern="^/edit(?: |$)(.*)")
async def edit_image_command(event):
    """أمر تعديل الصور"""
    chat = await event.get_chat()
    if chat.id == Config.OWNER_ID:
        await event.reply("**📤 الرجاء إرسال الصورة التي تريد تعديلها:**")
        # هنا يمكنك إضافة منطق استقبال الصورة ومعالجتها
    else:
        await event.reply("**⚠️ هذا الأمر متاح فقط للمالك.**")

@l313l.bot_cmd(pattern="^/create(?: |$)(.*)")
async def create_image_command(event):
    """أمر إنشاء صورة جديدة"""
    prompt = event.pattern_match.group(1)
    if not prompt:
        await event.reply("**✍️ الرجاء إدخال وصف الصورة:**\nمثال: `/create قطة لطيفة`")
        return
    
    await event.reply(f"**⏳ جاري إنشاء صورة مع الوصف:**\n`{prompt}`")
    # هنا يمكنك إضافة منطق إنشاء الصورة

@l313l.tgbot.on(CallbackQuery(data=re.compile(b"edit_image$")))
async def edit_image_callback(event):
    """زر تعديل الصور في القائمة"""
    await event.edit(
        """**🎨 تعديل الصور بالذكاء الاصطناعي**
        
**📤 أرسل الصورة التي تريد تعديلها**
**✍️ ثم أرسل وصف التعديل المطلوب**
