...
async def get_user_fragment_username(client, user_id):
    """
    يحصل على يوزر Fragment للمستخدم
    """
    try:
...
async def show_fragment(event):
    ...
    # البحث عن يوزر Fragment في المعلومات
    if hasattr(full_user, 'collectible_info'):
        fragment_username = full_user.collectible_info.collectible_id
        await event.reply(f"**🔖 يوزر Fragment:** @{fragment_username}")
    else:
