...
import re
import random
# في بداية الملف، أضف هذه المكتبات:
import aiohttp
import threading
import asyncio
import json
...
async def bot_start(event):
    ...
    if chat.id != Config.OWNER_ID:
        ...
        buttons = [
            ...
            [
                Button.inline("🖼️ إنشـاء صور", data="create_image_btn"),
                Button.inline("✏️ تعديـل صور", data="edit_image_btn")
            ],
            [
...
@l313l.tgbot.on(CallbackQuery(data=re.compile(b"create_image_btn$")))
async def handle_create_image_btn(event):
    await event.edit(
        """🎨 **إنشاء صورة بواسطة الذكاء الاصطناعي**
        
لإنشاء صورة جديدة:
1. أرسل /create
2. اتبع التعليمات

أو يمكنك استخدام الأزرار أدناه:""",
        buttons=[
            [Button.inline("بدء إنشاء صورة", data="start_create_image")],
            [Button.inline("رجوع", data="styleback")]
        ]
    )

@l313l.tgbot.on(CallbackQuery(data=re.compile(b"edit_image_btn$")))
async def handle_edit_image_btn(event):
    await event.edit(
        """✏️ **تعديل صورة بواسطة الذكاء الاصطناعي**
...
أو يمكنك استخدام الأزرار أدناه:""",
        buttons=[
            [Button.inline("بدء تعديل صورة", data="start_edit_image")],
            [Button.inline("رجوع", data="styleback")]
        ]
    )

@l313l.tgbot.on(CallbackQuery(data=re.compile(b"start_create_image$")))
async def start_create_image_handler(event):
    user_id = event.query.user_id
    dd.append(user_id)  # إضافة المستخدم لقائمة إنشاء الصور
    await event.edit(
        "🎨 **مرحباً بك في أداة إنشاء الصور**\n\n"
        "✍️ **الرجاء إرسال وصف الصورة المطلوبة:**\n"
        "(مثال: منظر طبيعي لغروب الشمس على البحر)\n\n"
        "❌ للإلغاء أرسل /cancel",
        buttons=[[Button.inline("رجوع", data="styleback")]]
    )

@l313l.tgbot.on(CallbackQuery(data=re.compile(b"start_edit_image$")))
async def start_edit_image_handler(event):
    user_id = event.query.user_id
    # يمكنك إضافة منطق خاص هنا
    await event.edit(
        "📤 **الرجاء إرسال الصورة التي تريد تعديلها أولاً**\n"
        "ثم أرسل وصف التعديلات المطلوبة\n\n"
        "❌ للإلغاء أرسل /cancel",
        buttons=[[Button.inline("رجوع", data="styleback")]]
    )
...
@l313l.bot_cmd(incoming=True, func=lambda e: e.is_private)
async def handle_image_creation(event):
    chat = await event.get_chat()
    
    # التحقق من أن المستخدم في وضع إنشاء الصور
    if int(chat.id) in dd:
        # إذا كانت الرسالة ليست نصاً، تخطي
        if not event.text or event.text.startswith('/'):
            return
        
        prompt = event.text
        if len(prompt.strip()) < 3:
            await event.reply("❌ الوصف قصير جداً. الرجاء إدخال وصف مفصل.")
            return
        ...
        await event.reply("⏳ جاري إنشاء الصورة... قد يستغرق الأمر دقائق")
        
        # بدء عملية إنشاء الصورة في خيط منفصل
        def create_image_thread():
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            
            # الحصول على حساب أو إنشاء جديد
            account = get_or_create_account(chat.id)
            
            if not account:
                loop.run_until_complete(event.reply("❌ فشل في إنشاء الحساب. الرجاء المحاولة لاحقاً."))
                loop.close()
                return
            ...
            if task_id:
                loop.run_until_complete(event.reply(f"✅ تم بدء إنشاء الصورة\n📝 رقم المهمة: {task_id}"))
                        ...
                        try:
                            loop.run_until_complete(
                                event.client.send_file(
                                    chat.id,
                                    filename,
                                    caption=f"✅ تم إنشاء الصورة بنجاح!\n📧 الحساب: {account['email']}\n📊 الاستخدام: {account.get('use_count', 0)}/5"
                                )
                            )
                        except Exception as e:
                            loop.run_until_complete(event.reply(f"❌ خطأ في إرسال الصورة: {str(e)}"))
                    else:
                        loop.run_until_complete(event.reply("❌ فشل في حفظ الصورة."))
                else:
                    loop.run_until_complete(event.reply("❌ فشل في إنشاء الصورة."))
            else:
                loop.run_until_complete(event.reply("❌ فشل في بدء عملية إنشاء الصورة."))
