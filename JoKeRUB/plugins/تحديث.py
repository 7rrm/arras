...
async def update_music(event):
    ...
    music_msg = await zzz4.edit(
        ...
        f"**•─────────────────•**\n"
        f"**•⎆┊اهـلا عـزيـزي** - {mention}\n"
        f"**•⎆┊يـتـم الان تـحـديـث مـيـوزك اراس**\n"
    ...
    try:
        ...
        # استنساخ المستودع الجديد
        process = await asyncio.create_subprocess_shell(
            "git clone https://github.com/jepthoniq/JepVc jepvc",
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.PIPE
        )
        await process.wait()
        ...
        if os.path.exists("jepvc/requirements.txt"):
            process = await asyncio.create_subprocess_shell(
                "pip install -r jepvc/requirements.txt --upgrade",
                stdout=asyncio.subprocess.PIPE,
                stderr=asyncio.subprocess.PIPE
            )
            await process.wait()
        ...
        # رسالة النجاح
        await music_msg.edit(
            f"ᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 𝗔𝗥𝗥𝗔𝗦 🝢 **تـم تـحـديـث مـيـوزك**\n"
            f"**•─────────────────•**\n"
            f"**•⎆┊اهـلا عـزيـزي** - {mention}\n"
            f"**•⎆┊تم تحديث ميوزك اراس بنجاح ✅**\n"
            f"**•⎆┊يمكنك الآن استخدام الميوزك**"
        )
        
    except Exception as e:
        await music_msg.edit(f"**✧︙ فشل تحديث ميوزك اراس:\n`{str(e)}`**")
