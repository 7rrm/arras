...
<tg-emoji emoji-id="{PREMIUM_EMOJI_ID}">👁️</tg-emoji> <b>هو فقط من يستطيع رؤيتها</b>'''
            ...
            buttons = [[Button.inline(fmm, data=f"{scc}_{timestamp}")]]
            
            # استخدم builder.article مع parse_mode
            from telethon.tl.types import InputBotInlineResultPhoto, InputBotInlineMessageText
            
            result = builder.article(
                title=f"همسة سرية لـ {zilzal}",
                description="همسة سرية - اضغط لفتحها",
                text=result_text,
                buttons=buttons,
                link_preview=False
            )
            ...
            bbb = [[Button.switch_inline(button_text, query=("secret " + gvarstatus("hmsa_id") + " "), same_peer=True)]]
