...
async def variable(event):
    ...
    if input_str == "تفعيل":
        ...
        # نسبة 20%
        zzz1 = await jep.edit("ᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 𝗔𝗥𝗥𝗔𝗦 🝢 **تـحـديـث مـيـوزك**\n**•─────────────────•**\n**⇜ جـارِ تـحـديـث مـيـوزك اراس . . .🌐**\n\n%𝟸𝟶 ▬▬▭▭▭▭▭▭▭▭")
        await asyncio.sleep(1)
        ...
        try:
            ...
            await zzz4.edit(f"**⌔∮ فشل تحديث مستودع الميوزك:\n`{str(e)}`**")
            return
    
    else:
        return await edit_delete(event, "**الاستخدام الصحيح:\n`.ميوزك تفعيل`\n`.ميوزك تعطيل`\n`.ميوزك تحديث`**")
