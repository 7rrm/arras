...
# ⚡⚡⚡ أسرع معالج ممكن (مع حساب وقت الاستجابة)
@l313l.on(events.NewMessage)
async def quotes_handler(event):
    # بدء حساب الوقت فور وصول الرسالة
    ...
    # حساب الوقت المستغرق حتى الآن
    elapsed_time = time.time() - start_time
    elapsed_ms = int(elapsed_time * 1000)  # تحويل إلى ملي ثانية
    
    # تنسيق الوقت المستغرق (أقل من ثانية أو أكثر)
    if elapsed_ms < 1000:
        time_str = f"⚡ استغرق الرد: `{elapsed_ms} ملي ثانية`"
        
        # إضافة تعليقات حسب السرعة
        if elapsed_ms < 50:
            time_str += " 🚀 (سرعة الضوء)"
        elif elapsed_ms < 100:
            time_str += " ⚡ (سريع جداً)"
...
async def load_enabled_chats(event):
    ...
    # هنا يمكن إضافة كود تحميل المجموعات المفعلة من قاعدة البيانات
    pass
