...
from telethon.tl.types import InputEmojiStatuses

@l313l.ar_cmd(
    pattern="جلب_ايموجيات_حزمه$",
    command=("جلب_ايموجيات_حزمه", plugin_category),
    info={
        "header": "جلب جميع إيموجيات حزمة معينة",
        "الاستخدام": "{tr}جلب_ايموجيات_حزمه بالرد على رابط الحزمة أو على إيموجي",
    },
)
async def get_all_emojis(event):
    replied_message = await event.get_reply_message()
    
    if not replied_message:
        return await edit_delete(event, "**⚠️ يرجى الرد على رابط الحزمة أو على إيموجي**", time=10)
    ...
    try:
        ...
        if replied_message.message.startswith("https://t.me/addemoji/"):
            ...
            # هنا يمكنك استدعاء إيموجيات الحزمة باستخدام اسم الحزمة
        ...
        if emoji_statuses:
            ...
            await edit_or_reply(
                event,
                f"**معرفات الإيموجيات في الحزمة:**\n" + "\n".join([f"`{emoji_id}`" for emoji_id in emoji_ids])
            )
        else:
            await edit_or_reply(event, "**❌ لم يتم العثور على إيموجيات في هذه الحزمة**")
    except Exception as e:
        await edit_or_reply(event, f"**⚠️ خطأ:** {str(e)}")
