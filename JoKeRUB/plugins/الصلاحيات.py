...
@l313l.ar_cmd(
    pattern="قفل تعديل الميديا$",
    command=("قفل تعديل الميديا", plugin_category),
    info={
        "header": "لقفل تعديل الميديا في المجموعة",
        "الوصف": "يقوم بقفل خاصية تعديل الميديا في المجموعة مع تفعيل كاشف التعديل الذكي",
        "الاستخدام": ".قفل تعديل الميديا"
    },
    groups_only=True,
    require_admin=True,
)
async def lock_media_edit(event):
    if not event.is_group:
        return await edit_or_reply(event, "**هذا الأمر يعمل فقط في المجموعات!**")
    ...
    # تفعيل كاشف التعديل الذكي
    update_lock(zed_id, "media_edit_detector", True)
    
    # إرسال رسالة التأكيد
    msg = (
        f"**ᯓ 𝗭𝗧𝗵𝗼𝗻 - حمـاية المجموعـات🛡 .**\n"
        f"⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆\n"
        f"• **تم قفل تعديل (النصوص + الميديا) لـ ميديا .. بنجـاح ☑️**\n"
        f"• **في المجموعـة : {chat_title}**\n"
        f"• **عبـر مراقبـة الاحداث في المجموعـة 👁‍🗨**\n\n"
        f"**ᯓ 𝗭𝗧𝗵𝗼𝗻 - حمـاية المجموعـات🛡 .**\n"
        f"⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆\n"
        f"• **تم تفعيـل كاشف التعديـل الذكـي .. بنجـاح ☑️**\n"
        f"• **في المجموعـة: {chat_title}**\n\n"
        f"• **مميـزات الكاشـف 💎:**\n"
        f"- مراقبة احداث المجموعة كل 5 دقائق ⏰\n"
        f"- التعرف ع الميديا المعدلة وحذفها فوراً 👁‍🗨\n"
        f"- التعرف ع الميديا (الجديدة) المعدلة عبر نص\n"
        f"- إشعارات لمجموعة السجل الخاصه بك\n"
        f"- بـ معلومات الشخص صاحب التعديل\n"
        f"- يقوم بجلب الميديا المعدلة لك عند إكتشاف أي تعديل\n"
        f"- والمزيد من المميزات اكتشفها بنفسك"
    )
    
    await edit_or_reply(event, msg)

@l313l.ar_cmd(
    pattern="فتح تعديل الميديا$",
    command=("فتح تعديل الميديا", plugin_category),
    info={
        "header": "لفتح تعديل الميديا في المجموعة",
        "الوصف": "يفتح خاصية تعديل الميديا في المجموعة مع تعطيل كاشف التعديل الذكي",
        "الاستخدام": ".فتح تعديل الميديا"
    },
    groups_only=True,
    require_admin=True,
)
async def unlock_media_edit(event):
    if not event.is_group:
        return await edit_or_reply(event, "**هذا الأمر يعمل فقط في المجموعات!**")
    ...
    # فتح تعديل الميديا
    update_lock(event.chat_id, "document", False)
    
    # تعطيل كاشف التعديل الذكي
    update_lock(event.chat_id, "media_edit_detector", False)
    
    # إرسال رسالة التأكيد
    msg = (
        f"**ᯓ 𝗭𝗧𝗵𝗼𝗻 - حمـاية المجموعـات🛡 .**\n"
        f"⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆\n"
        f"• **تم فتح تعديل (النصوص + الميديا) لـ ميديا .. بنجـاح ☑️**\n"
        f"• **في المجموعـة : {chat_title}**\n"
        f"• **تم تعطيل مراقبـة الاحداث في المجموعـة**"
    )
