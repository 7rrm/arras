...
# قم باستبدال دالة ytdl_download_callback بهذا الكود
...
async def ytdl_download_callback(c_q: CallbackQuery):
    ...
    # إرسال الرابط للبوت الخارجي
    bot_username = "W60yBot"
    
    try:
        # نرسل الأمر للبوت الخارجي
        await c_q.client.send_message(bot_username, yt_url)
        
        # ننتظر قليلاً حتى يقوم البوت بالرد
        await asyncio.sleep(5)
        
        # نبحث عن آخر رسالة من البوت تحتوي على ملف
        async for msg in c_q.client.iter_messages(bot_username, limit=3):
        ...
        await c_q.edit(
            ...
            f"<b>⌔╎جـرب إرسال الرابط يدويًا لـ @{bot_username}</b>",
            parse_mode="html"
        )
