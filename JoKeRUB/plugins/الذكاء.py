...
# 🔐 ضع مفاتيحك هنا (استخدم المفاتيح الجديدة بعد إنشائها)
CLOUDFLARE_API_TOKEN = "cfat_kYeWsSQX042Wm0VLBZvvQKMd7f3yRb7Tz8y5qk4V896c875a"  # استبدل بالمفتاح الجديد
ACCOUNT_ID = "a4f6064c06ade920c14fc1edefbed8d7"

async def ai_img_gen_cloudflare(prompt):
    """
    توليد صورة باستخدام Cloudflare Workers AI - مجاني وسريع
    """
    
    # ترميز النص للتأكد من خلوه من مشاكل
    encoded_prompt = prompt
    
    # استخدام نموذج Flux-1.1-schnell (سريع ومناسب للبوتات)
    url = f"https://api.cloudflare.com/client/v4/accounts/{ACCOUNT_ID}/ai/run/@cf/black-forest-labs/flux-1.1-schnell"
    ...
    payload = {
        ...
        "steps": 8,  # زيادة العدد يحسن الجودة (8-12 مناسب)
        "width": 1024,
        "height": 1024
    }
    
    try:
        print(f"🎨 جاري رسم: {prompt[:50]}...")
        ...
        if response.status_code == 200:
            ...
            if "result" in result and "image" in result["result"]:
                ...
                # تحويل base64 إلى بايتات لحفظها كصورة
                image_bytes = base64.b64decode(image_base64)
                
                return image_bytes
            else:
                print(f"خطأ: تنسيق الرد غير متوقع {result}")
                return False
        else:
...
# =========================================================== #
# أمر الرسم (معدل لاستخدام Cloudflare)
# =========================================================== #
@l313l.ar_cmd(pattern="ارسم ?(.*)")
async def draw_image(event):
    prompt = event.pattern_match.group(1)
    if not prompt:
        return await edit_or_reply(event, "**⎉╎ارسـل** `.ارسم` **+ وصف الصورة**\n**مثال:** `.ارسم قطة على جبل`")
    
    zzz = await edit_or_reply(event, "**🎨 جـاري رسـم الصـورة بواسطة الذكاء الاصطناعي ...**")
    
    image_data = await ai_img_gen_cloudflare(prompt)
    ...
    if image_data:
        ...
        await zzz.edit(f"**⎉╎عذرًا، لم أتمكن من رسم `{prompt}`.**\n\n**الأسباب المحتملة:**\n• النص طويل جداً\n• الكلمات غير واضحة\n• حاول مرة أخرى بكلمات مختلفة")
