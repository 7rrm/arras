...
# دالة للحصول على كلمات الأغنية من Genius
def get_lyrics(song_name, artist_name):
    genius = lyricsgenius.Genius("2gXaLY0JasK3TkdGuLmkyyBVAPgQEeZ-xDX1buYW1vSK76myd_4Ox7QQmkQacNBR")  # استبدل YOUR_GENIUS_API_KEY بمفتاح API الخاص بك من Genius
...
async def lyrics_cmd(event):
    ...
    if lyrics:
        ...
        await event.client.send_message(
            event.chat_id, 
            f"**الاغنية:** `{song_name}`\n**الفنان:** `{artist_name}`\n\n**عذرًا، لم يتم العثور على كلمات الأغنية.**", 
            reply_to=reply
        )
