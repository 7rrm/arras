...
# ==================== كلاس CustomParseMode (نفس كود الأوامر) ====================
...
async def on_afk(event):
    ...
    if AFK_.USERAFK_ON and not (await event.get_sender()).bot:
        ...
        sender_name = sender.first_name if sender else "المستخدم"
...
async def _(event):
    ...
    if not AFK_.USERAFK_ON:
        ...
        if AFK_.reason:
            await edit_delete(
                event, f"✅ **تم تفعيل وضع السليب**\n✧︙ السبب: {AFK_.reason}", 5
            )
        else:
            await edit_delete(event, f"✅ **تم تفعيل وضع السليب**\n✧︙ تم التفعيل بدون سبب", 5)
        if BOTLOG:
            current_time = datetime.now().strftime("%Y-%m-%d %I:%M:%S %p")
            if AFK_.reason:
                await event.client.send_message(
