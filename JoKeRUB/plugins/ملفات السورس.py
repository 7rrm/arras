...
@l313l.ar_cmd(
    pattern="مسح_كامل$",
    command=("مسح_كامل", plugin_category),
    info={
        "header": "مسح شامل لجميع الأوامر في السورس",
        "usage": "{tr}مسح_كامل",
    },
)
async def full_scan(event):
    "مسح شامل لجميع الأوامر في السورس"
    await edit_or_reply(event, "**🔍 جاري المسح الشامل...**")
        ...
        for file in files:
            if file.endswith('.py'):
                ...
                try:
                    ...
                    # البحث عن جميع التطابقات المحتملة
                    patterns_to_search = [
                        r'pattern\s*=\s*["\']([^"\']+)["\']',
                        r'command\s*=\s*["\']([^"\']+)["\']',
                        r'\.ar_cmd\s*\(\s*["\']([^"\']+)["\']',
                    ...
                    for pattern in patterns_to_search:
                        ...
                        if matches:
                            for match in matches:
                                if match and match not in ['tools', 'utils', '']:
                                    file_commands.append(match)
                    
                    if file_commands:
                        results[file] = list(set(file_commands))  # إزالة التكرارات
    ...
    # إنشاء التقرير
    if results:
        report = "**📊 تقرير المسح الشامل:**\n\n"
        total_files = len(results)
        total_commands = sum(len(cmds) for cmds in results.values())
        
        report += f"**• الملفات الممسوحة:** {total_files}\n"
        report += f"**• الأوامر المكتشفة:** {total_commands}\n\n"
        
        # عرض أول 10 ملفات مع أوامرها
        report += "**🔸 الملفات الرئيسية:**\n"
        
        for i, (file, cmds) in enumerate(list(results.items())[:10]):
            report += f"\n**📁 {file}** ({len(cmds)} أمر):\n"
            for cmd in cmds[:5]:  # عرض أول 5 أوامر فقط
                report += f"  - `{cmd}`\n"
            if len(cmds) > 5:
                report += f"  - ... و {len(cmds)-5} أمر آخر\n"
        
        if total_files > 10:
            report += f"\n**📌 و {total_files-10} ملف آخر...**"
        
        await edit_or_reply(event, report)
        
        # حفظ التقرير الكامل في ملف
        full_report = "**📊 التقرير الكامل للمسح:**\n\n"
        ...
        filename = "تقرير_المسح_الكامل.txt"
        with open(filename, 'w', encoding='utf-8') as f:
            f.write(full_report)
        ...
        await event.client.send_file(
            event.chat_id,
            filename,
            caption="**📋 التقرير الكامل لجميع الأوامر**"
        )
        os.remove(filename)
